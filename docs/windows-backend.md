...
Window mode confirms on click. A valid preview click requests a fresh full-resolution native render, stores that exact frame in the selection result, and destroys the overlay immediately; the existing output worker then publishes it to the clipboard. When Windows integrity isolation denies `PrintWindow`, the preview and selection instead retain the same full-resolution crop from the frozen DXGI desktop frame. Empty space leaves the overlay open. Full-display and region tools continue to use Capture or Enter for confirmation.
...
Confirmed regions use absolute physical screen coordinates and are checked before a tight BGRA crop is allocated. Window candidates retain their native `HWND` and DWM extended-frame bounds; `PrintWindow(PW_RENDERFULLCONTENT)` rendering is isolated behind a 350 ms response timeout with at most two timed-out native calls left in flight. If native rendering fails, Captastic uses those bounds to crop the already-frozen DXGI frame, removes the shadow area, and restores rounded-corner alpha so elevated windows remain selectable without elevating Captastic. Because this compatibility image comes from the composed desktop, it can contain a foreground occluder. Windows Graphics Capture remains the planned unoccluded compatibility path for elevated, GPU-heavy, protected, or otherwise unsupported windows.
